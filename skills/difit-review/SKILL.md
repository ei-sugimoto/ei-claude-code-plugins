...
echo "DIFIT_MONITOR_TIMEOUT"; exit 1
```

`SURF` / `BSURF` は Step 2 / Step 3.5 で取得した値に置き換える。監視を起動したら、ユーザーに「ブラウザサーフェスを閉じたら自動で続けます」と伝えて **このターンを終える**。背景プロセスの終了通知が来たら Step 5 へ。
